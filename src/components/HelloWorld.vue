<template>
  <center>
    <div class="ckeditor_width" style=" border-style: groove; border-width: 1px;">
    ...
    <div>目前字數：
              <span :class="[{ red_word: word_count < 300 }
              ]">{{word_count}}
              </span>
              <br>(目標字數 300~500。紅字就是要換字)
    </div>
  </center>
</template>
...
export default {
  ...
  data() {
    return {
      illegal_word: ['整合', '加值', '應用', '擴充', '評估', '試作', '分析', '規劃', '研習', '研討', '實用化', '經營模式', '演進',
        '協助', '維護', '維運', '管理', '網頁', '市場', '市調', '市場調查', '成本', '撰寫', '支援', '改良', '提昇', '提升', '昇版',
        '升級', '衍生', '生產', '量產', '產製', '技術支援', '行銷', '行政作業', '管理作業', '審標', '策略', '設備擴充', '更新', '客制化',
        '廣告', '客戶', '推廣', '諮詢', '產品服務', '技術服務', '品質改善', '品保', 'CMMI作業', '樣測', '驗收', '教育訓練', '功能加強',
        '功能提昇', '功能新增', '客戶面', '市場面', 'ADSL', 'MOD', '1288', '委託案', '研擬', '規格', '草案', '採購', '展示', '會議',
        'LDAP', '提案', '研究所', '銷售', '合作'
        ],
      word_count: 0,
...
// Gateway間是否已建立好加密通道，及方便滑鼠移至整合加密通道上再進一步看到詳細的起迄點及加密通道資訊。<br><span style="background-color:red">welcome you </span>',
  ...
  watch: {
    editorData (val) {
      ...
      var clean_html_str = val.replaceAll(/<[^>]*>/g, "").replaceAll("&nbsp;", "").replaceAll("&amp;", "&")
      ...
      var tmp_val = val;
      // 以上要不找出">整合 喔
      var replace = '([^>]|[^;"]>)(' + this.illegal_word.join('|')+ ')';
      var re = new RegExp(replace, "g");
      var found = val.match(re);
      if (found) {
        tmp_val = val.replace(re, '$1<span style="background-color: red; color: white;">$2</span>');
        this.editorData = tmp_val;
      }
      // console.log(this.editorData);
      
      replace = '(;">)(' + this.illegal_word.join('|')+ ')([^<]+)(</span>)';
